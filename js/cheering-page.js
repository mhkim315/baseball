    ...
    if (section.songs && section.songs.length) {
      ...
      for (const song of section.songs) {
        ...
        if (song.youtubeUrl) {
          ...
          a.setAttribute("aria-label", `${song.name} 참고 영상`);
    ...
    <section class="panel" aria-label="그림으로 보는 입문">
      ...
      <div class="rules-gallery">
        <figure class="figure-block">
          <img src="picture/rules/field.jpg" alt="야구장 전경 — 수비 포지션과 전광판 등이 표시된 안내 그림" loading="lazy" />
          <figcaption class="caption">누가 어디 서 있는지, 전광판은 어디인지 한번 잡아 두면 경기가 훨씬 따라가기 쉬워요.</figcaption>
        </figure>
        <figure class="figure-block">
          <img src="picture/rules/scoring.jpg" alt="야구장 탑다운 뷰 — 주자가 베이스를 돌아 홈을 밟을 때 득점이 1점 올라가는 모습" loading="lazy" />
          <figcaption class="caption">화살표 방향대로 1·2·3루를 거쳐 홈 플레이트를 밟으면 그때 팀 점수가 1점 올라가요. 주자 여러 명이 있으면 홈을 밟은 만큼 점수가 쌓여요.</figcaption>
        </figure>
        <figure class="figure-block">
          <img src="picture/rules/strike.jpg" alt="스트라이크와 볼 — 노스윙과 스윙 시 비교" loading="lazy" />
          <figcaption class="caption">스윙을 안 했을 때: 공이 스트라이크 존을 지나가면 스트라이크, 아니면 볼이에요. 스윙을 했으면(헛스윙) 존 안·밖이랑 상관없이 스트라이크.</figcaption>
        </figure>
        <figure class="figure-block">
          <img src="picture/rules/check-swing.jpg" alt="볼·스트라이크 판정과 체크스윙 여부를 설명하는 그림" loading="lazy" />
          <figcaption class="caption">"진짜 스윙이었나?" 스윙도중 멈추게 되면 심판이 배트가 얼마나 나갔는지 봐요. 애매하면 비디오 판독이 나올 수 있어요.</figcaption>
        </figure>
        <figure class="figure-block">
          <img src="picture/rules/walk-hit-by-pitch.jpg" alt="사구와 볼넷 — 안타 없이 타자가 출루하는 두 가지 예" loading="lazy" />
        ...
        <figure class="figure-block">
          <img src="picture/rules/batted-ball.jpg" alt="파울볼, 홈런, 플라이볼, 안타 네 가지 타구 예시" loading="lazy" />
        ...
        <figure class="figure-block">
          <img src="picture/rules/force-tag.jpg" alt="포스아웃과 태그아웃의 차이를 보여주는 그림" loading="lazy" />
        ...
        <figure class="figure-block">
          <img src="picture/rules/umpire.jpg" alt="세이프, 아웃, 스트라이크, 볼 — 심판 수신호 안내" loading="lazy" />
...
async function main() {
  const team = initShell("cheering", "응원");
  try {
