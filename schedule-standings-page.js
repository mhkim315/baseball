...
function renderStandings(standings, team) {
  ...
  for (const row of standings?.rows || []) {
    ...
    tr.innerHTML = `<td>${escapeHtml(String(row.rank || ""))}</td><td>${escapeHtml(row.teamName || "")}</td><td>${escapeHtml(wr)}</td><td class="standings-wlt">${escapeHtml(compactWlt(row.wlt))}</td><td>${escapeHtml(gb)}</td><td>${escapeHtml(row.streak || "")}</td>`;
