...
def post_kbo(url: str, fields: dict[str, Any]) -> Any:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "Mozilla/5.0 (baseball-refac-local/1.0)",
        },
    )
...
def fetch_kbo_lineup(game_id: str, season_id: str) -> list[Any]:
...
def expected_lineups_from_kbo(team: dict[str, Any], iso_date: str, naver_game: dict[str, Any]) -> tuple[list[dict[str, Any]], list[dict[str, Any]], str | None]:
    ...
    # KBO response order: [LINEUP_CK], [home meta], [away meta], [home table], [away table].
    home_rows = table_rows(data[3]) if len(data) > 3 else []
    away_rows = table_rows(data[4]) if len(data) > 4 else []
    if is_home_kbo(team, kbo_game):
        return home_rows, away_rows, game_id
    return away_rows, home_rows, game_id
...
def build_for_team(team: dict[str, Any], start_date: str) -> None:
    ...
    ours, theirs, kbo_game_id = expected_lineups_from_kbo(team, day, game)
    if len(ours) < MIN_EXPECTED_BATTERS:
        print(f"{team['id']}: KBO expected lineup unavailable ({len(ours)} rows)")
    ...
    source = "kbo-expected" if ours else "kbo-expected-empty"
    out = {
        "meta": {
            "teamId": team["id"],
            "lineupDate": day,
    ...
    print(f"wrote {old_path} ({len(ours)} expected batters, starter={our_starter['name']})")
